 ...
 *   rate; a trashed athlete is out through the model's soft delete.
...
class PaymentsSummaryAction
{
    ...
    public function execute(Academy $academy, int $year, int $month): array
    {
        ...
        $expected = $this->feesOf($this->population($academy));
        $owing = $this->population($academy)->owing($year, $month, $asOf)->with(['feeTier', 'academy'])->get();
        ...
        return [
            ...
            'outstanding_cents' => (int) $owing->sum(static fn (Athlete $athlete): int => MonthlyFee::forAthlete($athlete) ?? 0),
    ...
    /**
     * The athletes a fee is owed from.
     *
     * @return Builder<Athlete>
     */
    private function population(Academy $academy): Builder
    {
    ...
    /** @param Builder<Athlete> $athletes */
    private function feesOf(Builder $athletes): int
    {
        return (int) $athletes->with(['feeTier', 'academy'])->get()
            ->sum(static fn (Athlete $athlete): int => MonthlyFee::forAthlete($athlete) ?? 0);
    }
}
