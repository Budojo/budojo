 ...
 *
 * Idempotency: a second POST with the same raw token returns 410 Gone
 * via a dedicated ValidationException, NOT 500. The accepted row is
 * still in the DB but no longer redeemable — clicking the email link
 * twice is a UX accident, not a bug.
 */
class AcceptAthleteInvitationAction
{
    ...
    public function execute(string $rawToken, string $password): array
    {
        ...
        // Race-window guard: the email could have been claimed by a
        // public /register between invite-send and invite-accept.
        ...
        return DB::transaction(function () use ($invitation, $password): array {
            $athlete = $invitation->athlete()->lockForUpdate()->first();
            \assert($athlete !== null);
