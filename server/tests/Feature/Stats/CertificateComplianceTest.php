...
function complianceCertificate(Athlete $athlete, ?string $expiresAt, bool $trashed = false): Document
{
...
function complianceFigures(object $test): array
{
...
it('counts athletes by their current certificate, and says what share is covered', function (): void {
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), '2027-03-01');
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), '2026-10-10');
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), '2026-05-01');
    ...
    expect(complianceFigures($this))->toBe([
        'total_active' => 4,
        'valid' => 1,
        'expiring' => 1,
        'expired' => 1,
...
it('counts an athlete once, by the certificate with the latest expiry', function (): void {
    ...
    // and one expired.
    // The valid one is uploaded FIRST — an old paper filed late — so "the
    // newest upload" would pick the lapsed one and fail here.
    $renewed = Athlete::factory()->for($this->academy)->create();
    complianceCertificate($renewed, '2027-05-01');
    complianceCertificate($renewed, '2024-05-01');
    // Nulls last: a later undated upload does not displace a dated one.
    $undatedLater = Athlete::factory()->for($this->academy)->create();
    complianceCertificate($undatedLater, '2027-05-01');
    complianceCertificate($undatedLater, null);

    expect(complianceFigures($this))
        ->toMatchArray(['total_active' => 2, 'valid' => 2, 'expired' => 0, 'missing' => 0, 'compliance_pct' => 100]);
});
...
it('reads an undated certificate and a deleted one as no certificate', function (): void {
    ...
    // academy-wide figure.
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), null);
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), '2027-03-01', trashed: true);

    expect(complianceFigures($this))->toMatchArray(['total_active' => 2, 'missing' => 2, 'valid' => 0]);
});
...
it('counts only active athletes, and only this academy', function (): void {
    ...
    complianceCertificate(Athlete::factory()->for($this->academy)->create(['status' => AthleteStatus::Inactive]), '2027-01-01');
    complianceCertificate(Athlete::factory()->for(userWithAcademy()->academy)->create(), '2027-01-01');
    ...
    // The one athlete who counts holds a lapsed certificate, so any of the
    // valid rows above leaking into their figure would show as `valid`.
    complianceCertificate(Athlete::factory()->for($this->academy)->create(), '2026-01-01');

    expect(complianceFigures($this))->toBe([
        'total_active' => 1, 'valid' => 0, 'expiring' => 0, 'expired' => 1, 'missing' => 0, 'compliance_pct' => 0,
    ]);
});
