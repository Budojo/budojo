...
it('returns an empty list when no fingerprints are configured', function (): void {
    Config::set('twa.package_name', 'it.budojo.app');
    Config::set('twa.sha256_fingerprints', []);
...
it('returns an empty list when the package name is missing', function (): void {
    Config::set('twa.package_name', '');
    Config::set('twa.sha256_fingerprints', ['AA:BB:CC']);
...
it('renders both fingerprints when Play App Signing has added a second one', function (): void {
    ...
    // both must validate against incoming TWA launches.
    Config::set('twa.package_name', 'it.budojo.app');
    Config::set('twa.sha256_fingerprints', [
    ...
    expect($body[0]['target']['sha256_cert_fingerprints'])->toHaveCount(2);
});
