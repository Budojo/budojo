...
- Husky's `commit-msg` hook fires inside the `Release` job in CI when `@semantic-release/git` auto-commits `CHANGELOG.md` back. The bot commit message has the full markdown changelog as the body, which contains long URLs like `[#120](https://github.com/m-bonanno/budojo/issues/120)` — those blow past commitlint's default `footer-max-line-length: 100` and the hook rejects the commit. semantic-release fails with `husky - commit-msg script failed (code 1)` *after* the changelog is generated but *before* the tag is pushed → no tag, no GitHub release, the workflow run is red but partial work landed locally on the runner. **The fix is two-sided** — Husky 9 does NOT auto-wrap user hooks, so `HUSKY=0` is just inert unless the hook script checks it. (a) Add `[ "${HUSKY:-1}" = "0" ] && exit 0` as the FIRST line of every `.husky/*` hook in the repo. (b) Set `HUSKY: "0"` on the Release step's `env:` block. Skipping (a) makes (b) a no-op. Scope (b) to the release step (not the workflow root) so dev-side enforcement on contributor `npm ci` runs stays intact.
