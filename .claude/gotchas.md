...
## Claude reviewer (claude-code-action)

- `anthropics/claude-code-action@v1` requires the **Claude Code GitHub App** installed on the repo (https://github.com/apps/claude). Without it the OIDC → app-token exchange returns 401 in 3s with `App token exchange failed: Claude Code is not installed on this repository`. `CLAUDE_CODE_OAUTH_TOKEN` alone is NOT enough — that authenticates the agent side; the app authenticates the GitHub side. One-time install per repo. First bit me on PR #793 run 26081115460.
- The action's **Workflow validation guard** refuses to run on any PR whose `.github/workflows/pr-claude-review.yml` doesn't match the version on the **default branch** (`main` here — NOT develop). Symptom: workflow run completes in 13s with `Action skipped due to workflow validation error. … workflow file must exist and have identical content to the version on the repository's default branch`, no agent execution, no comments posted. **Bootstrap:** the very PR that ADDS the workflow can't be reviewed by it (chicken-and-egg); the next release PR (develop → main) carries the workflow to main, after which every subsequent non-chore/docs/ci PR is reviewable. **Subsequent edits to pr-claude-review.yml hit the same guard** — every PR that touches the workflow file is silently skipped until a fresh release brings the edit to main. Either ship CI workflow tweaks via a separate ci/* PR that other PRs can merge in from develop, or schedule the test on the first PR AFTER the release.
- The bot login on inline + summary comments is `claude` (NOT `claude[bot]`), despite the action's README documenting `claude[bot]` as the `bot_name` default. Confirmed on PR #793 run 26083233537. `.claude/scripts/reviewer-replies.sh` defaults `BOT_LOGIN` accordingly; if a future action version flips the login back to `claude[bot]` (or to `github-actions[bot]`), override via `BOT_LOGIN='<actual>' ./reviewer-replies.sh …` and bump the script default in the same commit.
- The action requires self-hosted runners or paid GitHub-hosted minutes. On this repo (`runs-on: ubuntu-latest` initially), the job failed in 3s with `total_ms: 0` UBUNTU billable + zero job annotation — the runner was never allocated. Fix: align with `pr-checks.yml`'s `runs-on: [self-hosted, budojo]` (4 slimbook runners). Caught on PR #793 run 26081055245.
