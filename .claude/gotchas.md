...
- Forge deploy script that doesn't end with `$FORGE_PHP artisan queue:restart` → the long-lived `php artisan queue:work` daemon under supervisor caches its **classmap** at boot, pinned to whichever release directory `current` symlinked at the moment the process started. Subsequent deploys flip the symlink atomically to a new release dir, but the running PHP process keeps the OLD absolute paths. Any job whose class was **added after the worker booted** (a fresh `Mail`, `Job`, `Listener`) fails to autoload at `unserialize()` time → PHP returns `__PHP_Incomplete_Class` → calling any method on it explodes → `--tries=3` re-attempts run identical → the job lands in `failed_jobs`. The owner-side API call meanwhile returned 201 (the row was persisted, only the queued side-effect failed), so **nothing surfaces in the SPA** — only the recipient noticing the missing email. Recovery: `php artisan queue:restart && php artisan queue:retry <uuid|all>`. Permanent fix: edit the Forge deploy script (Site → Application → Deploy Script in the web UI) to end with `$FORGE_PHP artisan queue:restart`. Caught after the v1.18.0 athlete-invite slice landed: the worker had been alive ~3 months on a pre-v1.18 release that didn't contain `App\Mail\AthleteInvitationMail`, every owner-side click just silently dropped the email until the worker was restarted by hand. **Investigation entry point** when "the SPA returns OK but no email arrives": ssh in, `supervisorctl status` for worker uptime + `php artisan db:show --counts | grep -E 'jobs|failed_jobs'`. A failed_jobs row whose stack trace cites `releases/<old>/server/vendor/...` confirms the diagnosis at a glance.
