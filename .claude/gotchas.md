...
- Husky's `commit-msg` hook fires inside any CI step that runs `git commit` — e.g. an `@semantic-release/git`-style auto-commit of `CHANGELOG.md`. With commitlint enforcing `footer-max-line-length: 100`, multi-line bodies that include long markdown URLs (`[#120](https://github.com/m-bonanno/budojo/issues/120)` etc.) get rejected and the workflow fails with `husky - commit-msg script failed (code 1)`. **The fix is two-sided** — Husky 9 does NOT auto-wrap user hooks, so `HUSKY=0` is just inert unless the hook script checks it. (a) Add `[ "${HUSKY:-1}" = "0" ] && exit 0` as the FIRST line of every `.husky/*` hook in the repo. (b) Set `HUSKY: "0"` on the relevant CI step's `env:` block. Skipping (a) makes (b) a no-op. Scope (b) to the specific step (not the workflow root) so dev-side enforcement on contributor `npm ci` runs stays intact.
