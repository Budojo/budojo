...
describe('PrivacyPolicyComponent — canonical English /privacy (#291)', () => {
  ...
  it('language toggle points to the Italian translation at /privacy/it', () => {
    const { fixture } = setup();
    const root: HTMLElement = fixture.nativeElement;
    ...
    expect(itLink?.getAttribute('routerLink')).toBe('/privacy/it');

    // Active language ("English") rendered as non-clickable <strong>
    // with aria-current — same shape as the /privacy/it page.
    const activeMarker = toggle?.querySelector('[aria-current="true"]');
    expect(activeMarker?.textContent?.trim()).toBe('English');
  });

  it('links to the canonical sub-processors page (English-only today)', () => {
    const { fixture } = setup();
    const root: HTMLElement = fixture.nativeElement;
