...
describe('Check-in by class', () => {
  ...
  it("offers the day's classes as chips and opens on the one the clock points at", () => {
    ...
    cy.get('[data-cy="attendance-class-2"]').should('have.attr', 'aria-pressed', 'true');
    cy.get('[data-cy="attendance-class-1"]').should('have.attr', 'aria-pressed', 'false');
  });
  ...
  it('sends the class with the mark, and re-reads the room when another chip is tapped', () => {
    ...
    cy.get('[data-cy="attendance-class-1"]').should('have.attr', 'aria-pressed', 'true');
  });
