...
- **Design system polish.** Page widths and side padding now resolve through a small set of semantic tokens (`--budojo-container-content`, `--budojo-container-prose`, `--budojo-page-padding-*`) instead of being copy-pasted on every screen. No visible change — but adding a new screen now picks up the right chrome automatically.
