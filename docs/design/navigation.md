...
The **athlete rail shipped in #1110** (light surface, hairline right border): Feed · Academy · Attendance · More as rows, ➕ Create (reuses the shared create-sheet), and the profile chip → the `/dashboard/me/more` hub (where settings / language / sign-out live). "More" is a row that opens the hub, not an in-rail expander — so the demoted destinations (payments, documents, settings, public profile) sit on the same hub as on mobile. The **owner rail shipped in #1112** (same pattern): Home (academy) · Athletes · Community · More as rows, the brand → `/dashboard/academy` (the owner home, matching its aria-label — not the `/dashboard` index, which still redirects to the athletes roster), and the profile chip → the `/dashboard/more` hub (stats, attendance, activity, settings, support, what's-new, language, sign-out live there).
