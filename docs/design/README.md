...
- **[`screenshots/`](./screenshots/README.md)** — the visual inventory of every canonical page at three viewports, regenerated on demand via `npm run design:inventory`, and the desktop audit harness (`npm run design:audit`) that shoots every screen of the shipped app with its dialogs and states. Start here when building a new feature: scan the folder, find the closest existing pattern, reuse it. See the folder's README for the full feature-building playbook.
- **[`ux-audit-v2.61.md`](./ux-audit-v2.61.md)** — the screen-by-screen review of the desktop app at v2.61 (epic #1614): what each screen is for, what is right, what is wrong and against which rule, and what would make it better. One issue per intervention; the positive judgements and the deliberate non-interventions live only here. Mockups for the structural proposals under [`preview/audit-v2.61/`](./preview/audit-v2.61/README.md).
