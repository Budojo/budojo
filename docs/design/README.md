...
- **[`brand-kit/`](./brand-kit/README.md)** — 14 standalone SVG logo variants (glyph / wordmark / app-icon / favicon × dark / light / accent / currentColor) for handoff to designers, pitch decks, partner integrations. Geometry locked from `client/public/logo-glyph.svg` + `wordmark.svg` — derivative kit, not the SPA's source-of-truth.
