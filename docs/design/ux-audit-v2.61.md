...
| ID | Grade | Finding | Rule |
|---|---|---|---|
| ~~ATT-1~~ | — | ~~**The "Percentuale di presenze" card is 380 px of white with a number in the middle.**~~ **Withdrawn (#1635): an artefact of the harness, not of the app.** The card draws a doughnut and a 90-bar timeline from `series`, and both are there once the screen waits for its canvases before the shutter. The charts are built only after the summary fetch lands, which the ready-selector — page chrome — does not wait for; `settle()` returns at ~250 ms and `shoot()` then ticks the frozen clock past the entry animation, so a chart constructed *after* that tick starts animating on a clock that never moves again and is captured empty. The three `22-athlete-attendance*` screens now wait for both canvases. The frozen clock is kept: it is what makes these shots reproducible, and dropping it trades a blank canvas for a half-drawn one. | — |
