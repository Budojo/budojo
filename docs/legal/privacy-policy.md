...
- Piano di backup automatizzato della base dati **in implementazione prima della raccolta di dati reali in produzione**; lo stato corrente e la decisione fra opzioni (DigitalOcean Managed DB, `mysqldump` cron, snapshot del droplet) sono documentati al § 8 del [DPA template](./dpa-template.md) e in `docs/infra/production-deployment.md`.
