...
**Schema reale attuale (per onestà operativa)**: la catena di cascade DB è `users → academies (academies.user_id, FK cascade) → athletes (athletes.academy_id, FK cascade) → documents + attendance_records (entrambe FK athlete_id cascade)`, **più** `academies → documents` diretto (`documents.academy_id`, FK cascade — le carte dell'accademia, #1743: DAE, polizza RC, affiliazione, contratto d'affitto). Significato pratico: cancellare l'`User` owner dell'academy, in DB, fa il 90 % del lavoro automaticamente. Ciò che il cascade DB **non** copre: il file binario su disco di ogni `Document` (la cascade tocca la riga, non `storage/app/private/documents/<file>`), i token Sanctum (`personal_access_tokens.tokenable_type/_id` è polimorfico, non c'è FK), i job email in coda. Quindi:

1. **Documenti su disco** — PRIMA del cascade DB: per ogni `Document` raggiungibile dall'academy, `Storage::disk('local')->delete($document->file_path)`. **Due strade, non una**: quelli degli atleti (`documents.athlete_id → athletes.academy_id = X`) e quelli dell'accademia stessa (`documents.academy_id = X`, #1743). Equivalente runnable in tinker:
   ```php
   $academy = Academy::findOrFail($id);
   // Le carte degli atleti.
   $academy->athletes()->withTrashed()->with('documents')->get()->each(
       fn ($a) => $a->documents->each(fn ($d) => Storage::disk('local')->delete($d->file_path))
   );
   // Le carte dell'accademia (#1743) — NON raggiungibili dal giro sopra.
   $academy->documents()->withTrashed()->get()->each(
       fn ($d) => Storage::disk('local')->delete($d->file_path)
   );
   ```
   Il secondo giro è la metà che si dimentica: pende dall'accademia, non dall'atleta, e la stessa svista era in `PurgeAccountAction` fino a #1743 — il cascade DB toglieva le righe e lasciava i binari su disco per sempre.
