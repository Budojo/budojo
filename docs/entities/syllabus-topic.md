...
- `GET /api/v1/stats/syllabus/topics/{syllabusTopic}` — who has seen one topic this season (#1745): its held lessons, and the roster split into seen, seen once and never there while on the roster. Rules in [`../api/v1.yaml`](../api/v1.yaml) § `statsTopicExposure`
