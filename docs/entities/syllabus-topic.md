...
- **The programme of an athlete's own belt (#1861).** `GET /athletes/{athlete}/syllabus-coverage` carries a `grade` block: the in-season techniques expected up to their current belt (theirs and the grades below, plus those for everyone), how many the academy taught in a held lesson since they joined this season, and how many of those they were checked into. Null while no in-season topic names a belt, position or technique — the headline then already is the whole programme — and for a belt the ladder does not hold. **Only techniques are counted, each by its own `from_belt`:** an ungraded technique under a graded position counts as for everyone, the way the programme page shows and filters it, because nothing cascades. A promotion changes which items apply; the read is computed, so nothing is rewritten.
...
| `POST` | `/api/v1/academy/syllabus` | `name`, `kind` required; `parent_id` (a position), `in_season` and `from_belt` optional |
...
| `PATCH` | `/api/v1/academy/syllabus/{syllabusTopic}` | Partial — `name`, `kind`, `in_season`, `from_belt`, `sort_order` |
