...
- **The timetable supersedes planned changes (#1575).** While the academy has at least one class, every derivation deletes the rows with `effective_from > today`, and `POST /api/v1/academy/schedules` is refused with a 422: a change planned when the pills were the source would otherwise take over on its date while the column kept the derived days. Future changes are made on the timetable.
...
- **`Academy::currentSchedule()`** is sugar for `scheduleForDate(OperatorDay::today())` — the owner's today (#1963), the same day a training-days change is dated to and a scheduled change must be after.
- **Counting and listing scheduled days: `App\Support\ScheduledDays` (#1764).** The counting sibling of `scheduleForDate`, and since #1769 the only place scheduled days are counted: every attendance denominator (the month summary, the roster, the athlete tab) comes from it. The SPA keeps only the per-day predicate it paints the calendar with (`training-days.ts`). `between()` / `countBetween()` list or count the days in a window, `isScheduledOn()` answers for one day, `lastBefore()` walks back from a date (never including it). It reads the history and the closures once through `loadMissing('schedules')` / `loadMissing('closures')` and applies four rules: each day is resolved against the row in force **on that day**, never against `academies.training_days`; **null is not zero**, where null means no row was ever configured (hide the fraction) and zero means configured but nothing scheduled in the window; **nothing after today** counts, though today itself does; and **a closed day is never scheduled** ([`academy-closure.md`](./academy-closure.md), #1766), so a window that is all closure counts zero, not null. Never derive scheduled days from the timetable (`academy_classes`): it describes this week only. The missed-streak alert (`budojo:send-athlete-missed-streak-pushes`) reads its last three sessions through `lastBefore()`, and skips an academy whose history yields fewer.
