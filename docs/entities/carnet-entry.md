...
- `INDEX(carnet_id)` (#1722). The foreign key does not make one: SQLite does not index a referencing column on its own, and this line used to say it did. It serves the balance subqueries in `Carnet::scopeSpendableOn`, which runs once per athlete when the roster asks who owes the month, and `Carnet::scopeSpendableDuring` (#1760), which the arrears list runs once per past month.
...
- **One entry per lesson, or per day — the academy's call (#1576).** With a timetable an athlete can be checked into two classes on the same day, which is two `attendance_records`. Under `academies.carnet_entry_unit = lesson` (the default) that is two entries; under `day` the second session on a day already charged costs nothing, and the entry stays pinned to the day's first session — so the ledger keeps one row per entry spent under either rule, and `UNIQUE(attendance_record_id)` keeps meaning what it says. A day charged on one carnet is never charged again on the next, and removing the pinned session moves the entry to the session that remains rather than refunding a day that was still trained. Not a snapshot: changing the setting recounts every carnet the academy has sold.
...
| `UpdateAcademyAction` → `ReconcileAcademyCarnetsAction` | Changing `carnet_entry_unit` (#1576) changes what every ledger in the academy derives to. Every athlete holding a carnet is recomputed — the one write that spans the roster rather than one athlete. |
