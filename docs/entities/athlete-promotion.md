...
- `GET /api/v1/athletes/{athlete}/promotions` — paginated 20/page, owner-academy gated, ordered by `recorded_at DESC, id DESC`. Read-only. Carries `progression` (#1772) and `gaps` + `history_starts_at` (#1966) beside `data`; each row carries `is_opening`.
- `PATCH /api/v1/athletes/{athlete}/promotions/{promotion}` — #1431 PR 1 of 2. Body: `{ "recorded_at": "YYYY-MM-DD" }`. 422 when missing, malformed, or in the future; 403 when the promotion doesn't belong to the athlete in the path (mirrors the carnet double-check) or the caller lacks `athletes_create_update` in the athlete's academy. Also accepts `from_belt`, only to complete a starting row (#1966): 422 on any row that already has one, on a stripe row, or when equal to the row's `to_belt`.
- `POST /api/v1/athletes/{athlete}/promotion-skips` / `DELETE …/promotion-skips/{belt}/{stripes}` — "Saltato" on a ghost row and its undo (#1966); see [`athlete-promotion-skip.md`](./athlete-promotion-skip.md).
...
## The missing steps (#1966)

`GetPromotionGapsAction` reports the steps the history does not record, so the Promozioni tab can draw them as ghost rows: "Bianca · 4ª striscia — quando?". It rides on `GET /athletes/{athlete}/promotions` as `gaps` and `history_starts_at`, beside `progression`, computed over the whole history rather than the page. The walk itself is `App\Support\Promotion\PromotionGaps`, pure.

- **Rows are replayed oldest first.** Each row leaves the athlete on a belt and a count: a belt row on its `to_belt` with no stripes, a stripe row on its `belt_at_event` with its `to_stripes`. Rows sharing a moment (a day's backfill, all stored at midnight) go in the order the ladder climbs, not the order they were typed. The stripe reset a live promotion writes beside the belt row (a count that does not rise) is not a step.
- **The ladder is walked between them** with `RankLadder::nextStep()`, the same rule "Chi promuovere?" uses (#1841). Every step before the walk reaches the next row is missing, and so is every step from the last row to the belt and stripes the athlete holds today. The kids' rule is the same one too (`KidsEligibility`), asked about the belt held and the day of each stretch: someone who climbed the kids' grades at nine was a child on those steps.
- **Never a guess.** An interval the ladder cannot walk (a belt that went backwards, contradictory rows, a kids' grade an adult's ladder never passes) reports nothing.
- **Nothing before the first row.** The page says "prima del … la storia non è registrata" in one line. The same holds for the stripes an athlete was entered with: a starting row records the belt, not the count, so the count on that belt is taken as it comes.
...
- **Filling a gap is never refused.** A backfill that is exactly one of the gaps, dated in its window `(after, before]`, skips the chain check above: it is consistent by construction. Without that, the first stripe on a new belt after the old belt's stripes, or the middle one of several missing stripes filled first, would be refused. Every other backfill is checked as before.
- **"Saltato" is remembered** in `athlete_promotion_skips` ([`athlete-promotion-skip.md`](./athlete-promotion-skip.md)): a skipped step stops being reported until the skip is deleted.
