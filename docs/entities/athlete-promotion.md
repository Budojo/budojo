...
- **Every timeline opens with a starting row (#1771).** Creating an athlete — the form, the CSV import, the owner enrolling themselves — writes one `kind = belt` row through `OpenPromotionTimelineAction`, inside the create transaction: `from_belt = null`, `to_belt` = the belt they hold, dated **the owner's day the record is created** (`OperatorDay`, #1963), recorded by the person creating it. Without it an imported blue belt of nine years read "No promotions yet".
...
- **Whole days.** Since #1963 live rows (`AthleteObserver`) and the opening row are dated to the owner's day at midnight, like a backfilled one; rows written before carry the time of day they were saved. Either way the comparison is on dates: a belt given at 18:42 still counts that evening's session, and one given at 00:30 in Rome belongs to the owner's new day, not UTC's previous one. Rows on the same day are ordered by id.
