...
- **Who was in the room when a topic was taught** is one join, written once: `App\Support\TopicAttendance` returns the held lessons that named a set of topics, each with the distinct living athletes present — a deleted athlete's presences still make a lesson held, but the person is counted nowhere a list of people is shown (#1746). One evening is one exposure, however many topics it named. The technique drill-down (#1745) reads it.
