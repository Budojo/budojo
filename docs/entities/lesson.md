...
- **The adoption only fires when the day had one session to be at.** `AdoptUnattributedAttendanceAction` counts the academy's classes for that **weekday** and does nothing past one, because a presence that names neither of two evening classes could have been at either and a guess is indistinguishable from a fact afterwards. The count comes from the timetable rather than from existing lessons, since lessons are materialised one at a time and the first tagged would otherwise always look unique. A presence that already names a lesson is never reassigned. The `MarkAttendanceAction` path needs no such guard: the owner has explicitly submitted those athletes for that class.
