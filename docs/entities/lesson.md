...
- **A plan whose day has passed is unconfirmed, not taught (#1858).** The season map (`GET /api/v1/stats/syllabus/calendar`) splits the lessons that are not held in two, on the owner's today (`OperatorDay`, #1963): dated today or later is **planned**, earlier is **unconfirmed** — a plan nobody checked into. Tonight's lesson with nobody checked in yet is still planned. Neither is stored, and nothing deletes an unconfirmed plan: it is shown as such, not counted, and not treated as a failure.
- **The week's plan for the group names planned lessons only (#1863).** The season map's "copy the week's plan" builds plain text from the same payload on the client. It covers the week of the owner's today, or the next week once nothing is left ahead in this one. It prints one line per **planned** lesson still ahead, with each technique under its position. A held lesson has already happened, and an unconfirmed one is no longer a plan. Today's lesson counts as ahead only until its `starts_at` on the local clock: nobody checked in keeps it planned, but by the evening it is not news. An untimed one stays ahead. The class and topic names are the programme's own, untranslated, and the message covers the whole academy whatever filter the map is showing, because the group is every athlete. It is offered on the current season only. A week that runs past the season's end is not sent from this map, because the next season's lessons are not in its payload, so the map says the week opens the new season rather than that nothing is planned. When neither week has a plan, there is no message.
