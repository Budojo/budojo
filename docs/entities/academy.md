...
- **`name`, `address`, `logo_path`, `monthly_fee_cents`, `carnet_price_cents`, `carnet_entries`, `carnet_entry_unit`, `training_days`, and `season_start_month` are mutable** via `PATCH /api/v1/academy` (and the dedicated `/academy/logo` endpoints for the logo file). `slug` is intentionally immutable — renames keep the original permalink stable.
...
- `PATCH /api/v1/academy` — partial update of `name`, `address`, `monthly_fee_cents`, `carnet_price_cents`, `carnet_entries`, `carnet_entry_unit`, `training_days`, `season_start_month`
