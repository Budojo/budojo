...
- **A paid month freezes the athlete's carnet (#1364).** When a presence is marked, the existence of a row here for the attended `(year, month)` means no carnet entry is charged — the monthly fee already covers that month, and the carnet is a fallback rather than a parallel charge. The check is made at marking time and is not re-evaluated afterwards: marking a month paid later does not refund entries already spent, and deleting the payment does not retroactively charge them. See [`carnet-entry.md`](./carnet-entry.md).
