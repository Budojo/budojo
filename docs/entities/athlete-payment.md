...
- **A payment covers a period, not a month (#1382).** `(year, month)` is where the period **starts**; it runs `period_months` from there and may cross a year boundary. Every "is this month covered?" question goes through `AthletePayment::scopeCovering(year, month)` — the twelve-month table, `paid_current_month`, the `?paid` filter, the owner's digest, the overdue push, and the months the fee covers during carnet reconciliation. A caller writing the arithmetic itself is how two surfaces come to disagree.
