...
| `first_name_sort` | text | **generated** (VIRTUAL), read-only | The form the name is ORDERED and SEARCHED by (#1527) — `first_name` lower-cased with diacritics stripped. SQLite derives it; nothing writes it, so it cannot go stale. Never serialised: `AthleteResource` does not expose it. See `App\Support\NameFold` |
| `last_name_sort` | text | **generated** (VIRTUAL), read-only, **indexed** with `first_name_sort` | Same, for `last_name`. The composite index `athletes_name_sort_index` is `(last_name_sort, first_name_sort)` — the order the roster reads them in |
