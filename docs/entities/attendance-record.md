...
- **The monthly summary divides on the server (#1767).** Each row's `expected_count` is that athlete's own scheduled days in the month, from the day they joined (or their first presence in the month, if that came earlier: trial sessions entered after registering), with closures out and nothing after today (`ScheduledDays`). The month's `meta.training_days` is the academy's count for the header and is nobody's denominator. The page no longer counts days in the browser.
- **One denominator, everywhere (#1769).** The athlete tab's card (30/90/365 days) and its ring (`?month=YYYY-MM`) divide by the days the academy was **scheduled** to train, closures out, as the month summary and the roster do. The old card divided by *realised lesson days*: every date anyone was checked in. That made one athlete's number depend on other people, since an open mat on a Sunday added a day everyone else had missed. The trade is that a session held but never registered now counts against everyone, which is correct: a denominator that flexes with data entry cannot be compared month to month. The summary cache is versioned per academy and bumped by any closure or schedule change (`AttendanceSummaryCache`). Only the at-risk list still reads realised sessions, on purpose.
...
- **Who is drifting is measured against the athlete's own habit, in realised sessions (#1728).** `AtRiskAthletesAction` (`GET /stats/attendance/at-risk`) reads the academy's **realised sessions** — every distinct day anyone was checked in, deliberately not the scheduled days every rate divides by since #1769 — newest first, clipped per athlete at `joined_at`. Tonight counts for an athlete only once they are ticked: today becomes a session as soon as the first person is checked in, and not being checked in yet to a session still in progress is not an absence. Counting sessions instead of weeks makes it closure-proof: a closed August contributes nothing, so September does not flag the whole roster. `recent` is the athlete's last 8 sessions, `baseline` the 24 before those. One tier, the most severe that applies:
...
- **The at-risk list and the bell's missed-streak push disagree, by design.** `SendAthleteMissedStreakPushes` walks the **scheduled** days, from the schedule history minus closures (`ScheduledDays::lastBefore()`, #1764 / #1766), and asks about consecutive absence; the at-risk list walks the **realised** dates and asks about a ratio. A Friday the academy cancelled without recording a closure is a miss for the push and invisible to the list; recorded as a closure, it is invisible to both. They need different denominators; neither should be made to call the other.
...
- `GetAthleteAttendanceSummaryAction` — the athlete's rate over 30/90/365 days or a calendar month (#893, #1769), divided by the **scheduled** days in the window (`ScheduledDays`, closures out, up to today), from the joining day or the first presence if earlier.
