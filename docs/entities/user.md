...
| `attendance_peer_visible` | boolean | not null, default `true` | Opt-out flag for the peer-preview row on `/me/attendance/today` (#958). Default `true`. Flipping to `false` hides this user from peers viewing the page — the user's own attendance row is still created and counted toward leaderboard / streak aggregates, only the avatar+handle on the peer preview is suppressed. |
| `leaderboard_visible` | boolean | not null, default `true` | Opt-out flag for the monthly mat-hours leaderboard (#962). Default `true`. Flipping to `false` anonymises the row on `GET /api/v1/attendance/leaderboard` (`first_name = ''`, `last_name_initial = ''`, `anonymous = true`) but the session count still ranks faithfully — the order is preserved, only the identity is hidden. Since #1851 an owner caller also gets each visible row's full identity (`athlete`: full name, belt, photo) to draw it with the belt spine; an anonymised row carries none, for the owner too, and an athlete caller never gets it. |
...
| `locale` | string(2) | nullable | The language the user reads the app in (#1912): `en` or `it`, the `App\Enums\AppLocale` PHP enum, cast on the model. Written by `PATCH /api/v1/me/locale`, which the SPA calls whenever its language switch changes. Null until the SPA has said; the server then writes in English. |
...
- **Notifications are written in the user's language, when they are read (#1912).** The owner's alerts and digests (missed streak, unpaid athletes, medical-certificate and academy-document expiry) store their parameters in `data.params`, and `App\Support\NotificationText` writes the title and body from them in `users.locale`, for the inbox (`GET /me/notifications`) and the desktop's Windows notification (`budojo:list-desktop-notifications`) alike, from `lang/{en,it}/notifications.php`. Switching language re-words the whole inbox; nothing is re-stored. A kind it does not write, or a row whose parameters are incomplete, shows the stored English `title` and `body`, which every row keeps. The rows written before #1912 were given their parameters by a migration that reads them back out of the stored sentences.
- **The inbox can be ticked off (#1914).** `notifications.archived_at` moves a row out of the inbox into the archived view — nothing is deleted. The owner archives one row, or every read row at once, and gets an undo; the bell counts unread rows in the inbox only. Two kinds archive themselves when they stop being true: a missed-streak alert when that athlete checks in on or after the day it was raised (`ArchivesResolvedAlerts`, an `AttendanceRecord` observer; an old register filled in does not count), and an unpaid digest when a newer one for its month or a later one is delivered (`DeliverOwnerDigestAction` archives the ones that were there before it). The certificate and document digests never supersede each other: each lists only what crossed a threshold that day, so a newer one says nothing about the older.
...
- `PATCH /api/v1/me/locale` — the app's language, for the notifications the server writes (#1912)
