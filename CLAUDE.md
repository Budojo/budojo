...
- **End-to-end auto-merge (optional, requires `BUDOJO_BOT_TOKEN` PAT secret):** when set, the sweep job authors the `git push` and `gh pr create` with the bot identity, which fires `pr-checks.yml` on the resulting PR (the same recursion guard that suppresses GITHUB_TOKEN events does NOT suppress PAT events). The job then sets `gh pr merge --auto`, so the sweep merges itself the moment CI is green. Required scopes on the PAT: `Contents: write`, `Pull requests: write`. Without the secret the sweep falls back to the GITHUB_TOKEN identity and stays manual-merge — no regression vs. the original behaviour.
